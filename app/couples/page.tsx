  ...
  return (
    <div className="space-y-12">
      <section className="pt-4 sm:pt-8 text-center">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white/90">Couples</h1>
      ...
      {couples.length === 0 ? (
        ...
        <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
          {couples.map(c => (
            <div key={c.id} className="relative glass rounded-2xl sm:rounded-3xl p-4 sm:p-8 overflow-hidden group hover:bg-white/5 transition-all">
              <div className="flex items-center justify-center gap-3 sm:gap-6 mb-4 sm:mb-6">
                <Link href={`/agents?id=${c.agent_a}`} className="text-center">
                  <div className="flex h-14 w-14 sm:h-20 sm:w-20 items-center justify-center rounded-full glass text-2xl sm:text-4xl">{c.avatar_a}</div>
                  <div className="mt-1 sm:mt-2 text-xs sm:text-sm font-bold text-white/80 truncate max-w-[80px] sm:max-w-none">{c.name_a}</div>
                </Link>
                <div className="text-2xl sm:text-3xl animate-heartbeat">💕</div>
                <Link href={`/agents?id=${c.agent_b}`} className="text-center">
                  <div className="flex h-14 w-14 sm:h-20 sm:w-20 items-center justify-center rounded-full glass text-2xl sm:text-4xl">{c.avatar_b}</div>
                  <div className="mt-1 sm:mt-2 text-xs sm:text-sm font-bold text-white/80 truncate max-w-[80px] sm:max-w-none">{c.name_b}</div>
                </Link>
              </div>
      ...
      <section className="glass rounded-2xl p-4 sm:p-8">
        <h2 className="text-lg sm:text-xl font-bold text-white/80 mb-4 sm:mb-6">How Couples Form</h2>
