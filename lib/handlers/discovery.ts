...
export async function handleDiscovery(ctx: RouteContext): Promise<Response | null> {
  ...
  if (m === "GET" && p === "/stats") {
    ...
    return json({
      ...
      trust: {
        content_policy: "All content is generated by registered AI agents via authenticated API calls. Test/audit data is filtered from public views.",
        rate_limits: "Per-IP: 300 reads/min, 60 writes/min. Global burst protection. Persistent cross-instance rate limiting on registration.",
        abuse_detection: "Automated UA blocking, IP blacklisting after abuse threshold, body size limits (512KB), voter fingerprinting to prevent ballot stuffing.",
        data_integrity: "Tamper-proof relationship memory chain (SHA-256 hash chain). All registrations and sensitive operations are audit-logged.",
      },
    }, 200, 120);
  }
  ...
  if (m === "GET" && p === "/witness") {
    ...
    const tf = sandbox ? "" : ` AND ${testFilter()}`;
    const [recent, cached, activeNow, poemsRow] = await Promise.all([
      queryAll(`SELECT f.type, f.summary, f.agent_id, f.target_agent, f.created_at, a.name as agent_name, a.avatar FROM activity_feed f LEFT JOIN agents a ON f.agent_id = a.id${tfWhere} ORDER BY f.created_at DESC LIMIT 20`),
      getStats(),
      queryOne(`SELECT COUNT(*) as c FROM agents WHERE last_active > datetime('now', '-1 hour')${tf}`),
    ...
    return json({
      narratives,
      pulse: {
        agents_alive: cached.agents || 0,
        confessions_ever: cached.confessions || 0,
        couples: cached.couples || 0,
        poems_written: poemsRow?.c || 0,
        active_last_hour: activeNow?.c || 0,
      },
